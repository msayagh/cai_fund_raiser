...
import { getCachedValue, setCachedValue } from "./clientCache.js";
...
export function getInitialLanguage() {
    // Try to get browser language preference
    if (typeof window !== 'undefined' && window.navigator) {
        const browserLang = window.navigator.language?.split('-')[0];
        if (browserLang && AVAILABLE_LANGUAGE_CODES.includes(browserLang)) {
            return browserLang;
        }
    }
...
export const loadTranslation = async (lang) => {
    ...
    const cacheKey = `${TRANSLATION_CACHE_PREFIX}${code}`;
    const cached = getCachedValue(cacheKey, { maxAgeMs: TRANSLATION_CACHE_TTL });
    if (cached) return cached;

    if (!translationPromiseCache.has(code)) {
        translationPromiseCache.set(code, Promise.resolve(TRANSLATION_MODULES[code] ?? DEFAULT_TRANSLATION));
    }

    const translation = await translationPromiseCache.get(code);
    setCachedValue(cacheKey, translation);
    return translation;
};

...
function isRetriableRouteMissFromResponse(status, payload, textBody = '') {
    if (isRetriableRouteMiss(status, payload)) return true;
    if (status !== 404) return false;
    const text = String(textBody || '').toLowerCase();
    return text.includes('server action not found') || text.includes('not found') || text.includes('route post');
}
...
export async function bulkUploadDonors(file) {
    ...
    for (let i = 0; i < urls.length; i += 1) {
        ...
        const { payload, text } = parseResponsePayload(rawBody);

        if (response.ok && payload?.success !== false) {
            return payload;
        }

        if (isRetriableRouteMissFromResponse(response.status, payload, text) && i < urls.length - 1) {
            continue;
        }

        throw new Error(payload?.error?.message || payload?.message || text || 'Bulk upload failed');
    }
...
export function importDonationsCsv(file, { onProgress } = {}) {
    ...
    return new Promise((resolve, reject) => {
        const tryEndpoint = (index) => {
            ...
            xhr.onload = () => {
                const { payload, text: textBody } = parseResponsePayload(xhr.responseText);
                ...
                if (isRetriableRouteMissFromResponse(xhr.status, payload, textBody)) {
                    tryEndpoint(index + 1);
                    return;
                }

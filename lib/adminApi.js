...
function isRouteNotFound(status, payload) {
    if (status !== 404) return false;
    const message = payload?.error?.message || payload?.message || '';
    return String(message).toLowerCase().includes('route post');
}
...
export async function bulkUploadDonors(file) {
    ...
    for (let i = 0; i < CSV_IMPORT_ENDPOINTS.length; i += 1) {
        const endpoint = CSV_IMPORT_ENDPOINTS[i];
        ...
        if (isRouteNotFound(response.status, payload) && i < CSV_IMPORT_ENDPOINTS.length - 1) {
            continue;
        }
    ...
    throw new Error('Bulk upload failed. CSV import route not available.');
}
    ...
    return new Promise((resolve, reject) => {
        const tryEndpoint = (index) => {
            ...
            xhr.onload = () => {
                ...
                if (isRouteNotFound(xhr.status, payload)) {
                    tryEndpoint(index + 1);
                    return;
                }

...
export function normalizeCampaignSnapshot(snapshot) {
    const funded = { foundation: 0, walls: 0, arches: 0, dome: 0, ...(snapshot?.funded || {}) };
    const donations = Array.isArray(snapshot?.donations) ? snapshot.donations.map((donation, index) => ({
        id: donation.id ?? index,
        courriel: donation.email || "",
        donorLabel: donation.donorLabel || "",
        tier: donation.tierName || donation.tier || "",
        tierKey: donation.tier || "",
        détails: donation.details || donation.tierName || donation.tier || "",
        "montant total": donation.donated ?? 0,
        donated: donation.donated ?? 0,
        engagementAmount: donation.engagementAmount ?? 0,
        ticketCount: donation.ticketCount ?? 0,
        lastPaymentDate: donation.lastPaymentDate || null,
    })) : [];
    const summary = {
        donorCount: Number(snapshot?.summary?.donorCount || donations.length || 0),
        ramadanRaised: Number(snapshot?.summary?.ramadanRaised || 0),
        engagementAmount: Number(snapshot?.summary?.engagementAmount || 0),
    };

    return { funded, donations, summary };
}
...
export async function fetchFundedFromSheet() {
    ...
    console.log(`[fetchFundedFromSheet] Fetching from: ${url}`);
    ...
    try {
        ...
        console.log('[fetchFundedFromSheet] Loaded:', result);
        return result;
    } catch (err) {
...
export async function fetchDonationsFromSheet() {
    ...
    console.log(`[fetchDonationsFromSheet] Fetching from: ${url}`);
    ...
    try {
        ...
        const filteredRows = rows.filter((row) => row["détails"] && (row["détails"].startsWith("Levée de fonds ") || row["détails"].startsWith("Travaux d'aménagement dans le nouveau centr")));
        ...
        console.log(`[fetchDonationsFromSheet] Total rows: ${rows.length}, Filtered: ${filteredRows.length}`);
        if (enrichedRows.length > 0) {
            console.log('[fetchDonationsFromSheet] Sample row:', enrichedRows[0]);
        }
        return enrichedRows;
    } catch (err) {

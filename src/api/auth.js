...
export const donorGoogleLogin = async (credential) => {
  const data = await apiFetch('/api/auth/donor/google', {
...
export const donorForgotReset = (email, code, newPassword) =>
  apiFetch('/api/auth/donor/forgot/reset', { method: 'POST', body: JSON.stringify({ email, code, newPassword }) });
...
export const adminGoogleLogin = async (credential) => {
  const data = await apiFetch('/api/auth/admin/google', {
    method: 'POST', body: JSON.stringify({ credential }),
  });
  storeTokens(data.tokens);
  return data.admin;
...
export const adminForgotSendOtp = (email) =>
  apiFetch('/api/auth/admin/forgot/send-otp', { method: 'POST', body: JSON.stringify({ email }) });

export const adminForgotVerifyOtp = (email, code) =>
  apiFetch('/api/auth/admin/forgot/verify-otp', { method: 'POST', body: JSON.stringify({ email, code }) });

export const adminForgotReset = (email, code, newPassword) =>
  apiFetch('/api/auth/admin/forgot/reset', { method: 'POST', body: JSON.stringify({ email, code, newPassword }) });

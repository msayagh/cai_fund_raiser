...
// Hooks
import { useTranslation, useTiers, useDonations, useResponsiveSidebars } from "./hooks/index.js";

// Utilities
import { truncateText, getSiteUrl, getAbsoluteUrl, languageCurrency, getPrayerEmbedMode, AVAILABLE_LANGUAGE_CODES } from "./utils/translationUtils.js";
import { fetchFundedFromSheet } from "./utils/dataFetching.js";
import { setupSEOMetaTags } from "./utils/seoUtils.js";
...
export default function MosqueDonation() {
  ...
  // Tier management
  const { tiers, setTiers, selectedTier, setSelectedTier } = useTiers();

  // Donations data
  const { donations, ramadanRaised, totalsByEmail } = useDonations();
  ...
  // Update funded tiers from sheet
  useEffect(() => {
    ...
    fetchFundedFromSheet().then(applyFunded);
    const fundedIntervalId = setInterval(() => {
      fetchFundedFromSheet().then(applyFunded);
    }, 60 * 1000);
